...
class ClientCasesTable extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<ClientsCubit, ClientsState>(builder: (context, state) {
      ...
      final cases = clientsCubit.casesList?.dentistCases ?? [];

      if (state is ClientCasesLoading && cases.isEmpty) {
        return const Center(child: CircularProgressIndicator());
      }
      if (state is ClientCasesError && cases.isEmpty) {
        return const Center(child: Text('حدث خطأ أثناء تحميل الحالات'));
      }
                  ...
                  (index) {
                    ...
                    return DataRow(
                      cells: [
                        ...
                        DataCell(
                            Center(child: CustomText(text: caseitem.status!))),
                        DataCell(Center(
                            child: CustomText(
